...
void abbababa::printArg(FILE *argFile){
  ...
  fprintf(argFile,"\t-blockSize\t\t%d\tnumber of based in a block\n",blockSize);
...
void abbababa::getBlockNum(int pos){
...
void abbababa::run(funkyPars *pars){
  ...
  if(doAbbababa==1){//random number read
    for(int s=0;s<pars->numSites;s++){
      if(pars->keepSites[s]==0)
	continue;
