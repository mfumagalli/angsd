...
void error::printArg(FILE *argFile){
  fprintf(argFile,"---------------------\n%s:\n",__FILE__);
...
void error::consolidate(funkyPars *p) {
...
double likeFixedMinorError_wrapper_tsk(const double *para,const void *dats){ //est error
  ...
  for(int s=0;s<loc_pars->numSites;s++){
    ...
    if(keepInd>0)
      totalLogLike += frequency::likeFixedMinor(frequency::emFrequency(loglike3[s],loc_pars->nInd,loc_pars->emIter,loc_pars->EM_START,keeps,keepInd),loglike3[s],loc_pars->nInd);
  }
...
void error::killGlobalErrorProbs(double ****errorProbs){
  if(errorProbs==NULL)
    return;
  int nG=3; //genotypes
...
void error::getOptions(argStruct *arguments){
  ...
  if(doError && it!=0 && (it==7 && doCounts==0) ){
...
error::error(const char *outfiles,argStruct *arguments,int inputtype){
  ...
  if(arguments->argc==2){
    if(!strcmp(arguments->argv[1],"-doError")){
      printArg(stdout);
      exit(0);
    }else
      return;
...
void error::print(funkyPars *pars){
  if(doError==0)
    return;
...
void error::clean(funkyPars *pars){
  if(doError==0)
    return;
...
void error::likeFixedMinorError_bfgs_tsk2(double **errors,int numSites,int nInd,int *major,int *minor,suint **counts,int emIter,double EM_START){ //est error
  double start[12];
  double lbound[12];
  double ubound[12];
  int lims[12];
...
void error::setMajorMinor(suint **counts,char *major,char *minor,int nInd,int nSites){
...
double **** error::generateErrorPointers(double **error,int nG,int nA) {
  //  if(errorProbs==NULL)//
  double ****errorProbs= new double***[3];
  //estimates the probabilities for the four bases given the major and minor
  //The function is only called once and is of no importance speedwise
