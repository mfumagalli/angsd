...
void hwe::print(funkyPars *pars){
  ...
  for(int s=0;s<pars->numSites;s++){
    ...
    gzprintf(outfileZ,"%s\t%d\t%c\t%c\t%f\t%f\t%f\t%f\n",header->name[pars->refId],pars->posi[s]+1,intToRef[pars->major[s]],intToRef[pars->minor[s]],freq->freq[s],hweStruct->freq[s],hweStruct->F[s],2*hweStruct->like0[s]-2*hweStruct->likeF[s]);
