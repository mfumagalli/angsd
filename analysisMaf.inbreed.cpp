...
void frequency::getOptions(argStruct *arguments){
  ...
  if(indFName ==NULL){
    indF = new double [arguments->nInd];
    for(int i=0;i<arguments->nInd;i++) indF[i] =0;
  }else
...
double frequency::likeNoFixedMinor(double p,double *logLikes,int numInds,int major){
  ...
  for(int i=0;i<numInds;i++){
    for(int j=0;j<3;j++) 
      partialLike[j]+=angsd::addProtect3(
					 logLikes[i*10+angsd::majorminor[major][major]] + log(pow(1-p,2) + (1-p)*p*indF[i]),
					 logLikes[i*10+angsd::majorminor[major][iMinor[j]]] + log(2*(1-p)*p - 2*(1-p)*p*indF[i]),
					 logLikes[i*10+angsd::majorminor[iMinor[j]][iMinor[j]]] + log(pow(p,2) + (1-p)*p*indF[i]));
  }
...
double frequency::likeFixedMinor(double p,double *logLikes,int numInds){
  double totalLike=0;
  for(int i=0;i<numInds;i++){
    totalLike+=angsd::addProtect3(
				  logLikes[i*3+0] + log(pow(1-p,2) + (1-p)*p*indF[i]),
