...
double factln(int n)
{
  ...
  if (n < 0) fprintf(stderr,"Negative factorial in routine factln: %d \n", n);
  if (n <= 1) return 0.0;
...
realSFS::realSFS(const char *outfiles,argStruct *arguments,int inputtype){
  ...
  if(arguments->argc==2){
    if(!strcmp(arguments->argv[1],"-doSaf")){
      printArg(stdout);
      exit(0);
    }else
      return;
  ...
  if(doSaf==3){
    ...
    gzprintf(theta_fp,"#Chromo\tPos\tWatterson\tPairwise\tthetaD\tthetaSingleton\tthetaH\tthetaL\n");
    aConst=0;
...
void realSFS::algoJoint(double **liks,char *anc,int nsites,int numInds,int underFlowProtect, int fold,int *keepSites,realRes *r,int noTrans) {
  // fprintf(stderr,"[%s]\n",__FUNCTION__);
  int myCounter =0;
  if(anc==NULL||liks==NULL){
    fprintf(stderr,"problems receiving data in [%s] will exit (likes=%p||ancestral=%p)\n",__FUNCTION__,liks,anc);
...
void realSFS::algoGeno(int refId,double **liks,char *major,char *minor,int nsites,int numInds,kstring_t *sfsfile,int underFlowProtect,int *posi,int *keepSites,double *pest) {
  assert(pest!=NULL);
  ...
  for(int it=0; it<nsites; it++) {//loop over sites
	...
	for(int r=j;r<=2*numInds-2+j;r++) {
	  // fprintf(stderr,"tmp1: r=%d index: %d\n",r,r-j);
	    ...
	    if(underFlowProtect)
	      tmp1 = angsd::addProtect2(tmp1, log(myComb2(numInds,r,j))+ asdf[r-j]+pest[r]);
	    else
	      tmp1 += myComb2(numInds,r,j)*(asdf[r-j])*exp(pest[r]);
	...
	for(int r=2-j;r<=2*numInds-j;r++){
	  ...
	  if(underFlowProtect)
	    tmp2 = angsd::addProtect2(tmp2,log(myComb2(numInds,r,2-j))+ asdf[(2*(numInds-1))-(r-2+j)]+ pest[r]);
	  else
	    tmp2 += myComb2(numInds,r,2-j)*(asdf[2*(numInds-1)-(r-2+j)])*exp(pest[r]);
	}
