...
realSFS::realSFS(const char *outfiles,argStruct *arguments,int inputtype){
  ...
  if((doSFS!=0||doRealSFS!=0)&&doThetas==0){
    outfileSFS =  aio::openFile(outfiles,SFS);
    outfileSFSPOS =  aio::openFile(outfiles,SFSPOS);
  }else{
    fpgz = aio::openFileGz(outfiles,THETAS,"w6h");;
...
void realSFS::run(funkyPars  *p){
  ...
  if(doRealSFS==1)
    ...
    filipe::algoJoint(p->likes,p->anc,p->numSites,p->nInd,underFlowProtect,fold,p->keepSites,r,noTrans,doRealSFS,p->major,p->minor,freq->freq,filipeIndF,newDim);
  }
  if(doSFS)
