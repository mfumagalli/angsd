...
aHead *getHd(BGZF *gz){
  ...
  if(strncmp(head,magic,4)!=0) {
    fprintf(stderr,"[%s]\t-> problem with magic number\n",__FUNCTION__);
    exit(0);
  }
...
BGZF *openBAM(const char *fname){
  ...
  if((fp=bgzf_open(fname,"r"))==NULL ){
    fprintf(stderr,"nonexistant file: %s\n",fname);
    exit(0);
  }
  const char *str = strrchr(fname,'.');
  if(str&&strcmp(str,".bam")!=0){
...
int compHeader(aHead *hd1,aHead *hd2){
  ...
  if(hd1->n_ref!=hd2->n_ref){
    fprintf(stderr,"problem with number of chromosomes in header\n");
    exit(0);
  }
  for(int i=0;i<hd1->n_ref;i++){
    if(strcmp(hd1->name[i],hd2->name[i])!=0){
      fprintf(stderr,"problem with chromosome ordering");
      exit(0);
    }
    if(hd1->l_ref[i]!=hd2->l_ref[i]){
      fprintf(stderr,"problem with length of chromosomes");
      exit(0);
    }
  }
  return 0;
