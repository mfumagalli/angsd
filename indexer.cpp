...
void dalloc(tindex idx){
 ...
 int parse_region(char *extra,const aHead &hd,int &ref,int &start,int &stop) {
   ...
   //build chromonames -> chrID
   typedef std::map<const char*,int,ltstr> aMap;

   aMap myMap;
   for(int i=0;i<hd.n_ref;i++)
     myMap.insert(std::pair<char *,int>(hd.name[i],i));

   aMap::iterator it;
   if(strchr(extra,':')==NULL){//only chromosomename
     if((it = myMap.find(extra))==myMap.end()){
   ...
   if((it = myMap.find(tok))==myMap.end()){
       fprintf(stderr,"[%s.%s():%d] Problems finding chromo:%s\n",__FILE__,__FUNCTION__,__LINE__,extra);
       fflush(stderr);
       exit(0);
       return -1;
...
void getOffsets(char *bamname,const aHead *hd,iter_t &ITER,int ref,int start,int stop){
  //  fprintf(stderr,"chr=%d, start=%d\tstop=%d pointer=%p\n",ref,start,stop,ITER.dasIndex);
  //  fprintf(stderr,"chr=%d, start=%d\tstop=%d ITER.dasIndex.n_ref=%d\n",ref,start,stop,ITER.dasIndex->n_ref);
  if(ITER.dasIndex==NULL||ITER.dasIndex->loadedRef !=ref){
    //    fprintf(stderr,"reding bai");
    dalloc(ITER.dasIndex);
    ITER.dasIndex = getIdx(bamname,ref);//index file
  }
