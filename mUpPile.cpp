...
tNode initNodeT(int l,int refPos, int arypos){
  tNode d;
  d.l = d.l2 = d.m2 = 0;
  d.m = l;
  kroundup32(d.m);
...
nodePool mkNodes_one_sample(sglPool *sgl,nodePool *np,getFasta *gf) {
...
nodePoolT mkNodes_one_sampleT(sglPool *sgl,nodePoolT *np) {
  ...
  //initialize new nodes for the rest of the region
  for(int i=np->l;i<regionLen;i++)
    nds[i] = initNodeT(UPPILE_LEN,-1,-1);
  ...
  for( r=0;r<sgl->readIDstop;r++) {
	...
	if(opCode&BAM_CINS){
	  ...
	  tmpNode->insert[tmpNode->l2] = initNodeT(opLen,-9,-9);
	  for(int ii=0;ii<opLen;ii++){
	    char c = bam_nt16_rev_table[bam1_seqi(seq, seq_pos)];
...
void *setIterator1(void *args){
  ...
  getOffsets(rd->bamfname,rd->hd,rd->it,ref,start,stop);//leak
  return EXIT_SUCCESS;
}
 ...
 void setIterator1_thread(void *args,int index){
  ...
  stop = rd->regions.stop;
  ref = rd->regions.refID;
  getOffsets(rd->bamfname,rd->hd,rd->it,ref,start,stop);
