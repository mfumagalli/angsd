...
int make_bed(int argc,char **argv){
  //  fprintf(stderr,"[%s] \n",__FUNCTION__);
  if(argc==0){
  ...
  fprintf(stderr,"Has dumped files: \'%s\'\n\t\t  \'%s\'\n",outnames_bin,outnames_idx);
  bgzf_close(cfpD);
  fclose(fp);
...
myMap getMap(const char *fname){
  ...
  while(fread(&clen,sizeof(size_t),1,fp)){
    ...
    chr[clen] = '\0';
    datum d;
    fread(&d.nSites,sizeof(size_t),1,fp);
    fread(&d.fpos,sizeof(int64_t),1,fp);
...
kstring_t do_stat_main(perChr &pc,int step,int win,int nChr){
  ...
  begI=endI=0;
  fprintf(stderr,"pS: %d pE:%d\n",pS,pE);
  kstring_t str;
  str.l=str.m=0;
  str.s=NULL;
  if(pE>pc.posi[pc.nSites-1]){
    fprintf(stderr,"end of dataset is before end of window: end of window:%s last position in chr:%d\n",pE,pc.posi[pc.nSites-1]);
    return str;
  }
...
int do_stat(int argc, char**argv){
  if(argc==0){
    fprintf(stderr,"do_stat FILE -win -step -nChr [-r chrName]\n");
    exit(0);
  }
  char *base = *argv;
  char* outnames_bin = append(base,BIN);
  ...
  if(win==0||step==0||nChr==0){
    fprintf(stderr,"win,step and nChr must be different from zero\n");
    exit(0);
  }
  while(1){
    ...
    kstring_t str = do_stat_main(pc,step,win,nChr);
    fwrite(str.s,1,str.l,fpres);
    if(chr!=NULL)
      break;
    dalloc(pc);
  }
...
int fun(float a,float b){
  if(fabs(exp(a)-b)>1e-6){
    fprintf(stderr,"difference between binary dump and raw text is rarther big:%f vs %s\n",a,b);
    exit(0);
  }
...
int main(int argc,char **argv){
  if(argc==1){
    fprintf(stderr,"./a.out [make_bed|do_stat|validate_bed]\n");
    return 0;
  }
  fprintf(stderr,"zlibversion=%s zlibversion=%s\n",ZLIB_VERSION,zlib_version);
  --argc,++argv;
