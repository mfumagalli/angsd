...
void thorfinn::printArg(FILE *argFile){
  fprintf(argFile,"%s:\n",__FILE__);
  fprintf(argFile,"-doThorfinn\t%d\n",doThorfinn);
  fprintf(argFile,"-ref\t%s\n",refName);
  fprintf(argFile,"\n");
}

void thorfinn::getOptions(argStruct *arguments){
  //default
  doThorfinn=0;
  refName = NULL;
  //from command line
  ...
  if(doThorfinn==0)
    return;
  // fprintf(stderr,"doThorfinn=%d refName=%s\n",doThorfinn,refName);
...
thorfinn::thorfinn(const char *outfiles,argStruct *arguments,int inputtype){
...
void thorfinn::clean(funkyPars *pars){
  if(doThorfinn==0)
    return;

}

void thorfinn::print(funkyPars *pars){
  if(doThorfinn==0)
    return;
...
void thorfinn::run(funkyPars *pars){
