...
class plink:public general{
private:
  FILE *fp1;//tfam/fam/bim
  FILE *fp2;//tped/bed
public:
  int doPlink;
  plink(const char *outfiles,argStruct *arguments,int inputtype);
  ~plink();
...
void plink::printArg(FILE *argFile){
  fprintf(argFile,"------------------------\n%s:\n",__FILE__);
...
void plink::run(funkyPars *pars){
  if(doPlink==0)
    return ;
   
}

void plink::clean(funkyPars *pars){
  if(doPlink==0)
    return;
...
void plink::print(funkyPars *pars){
  if(doPlink==0)
    return;
...
void plink::getOptions(argStruct *arguments){
...
plink::plink(const char *outfiles,argStruct *arguments,int inputtype){
  fp1=fp2=NULL;
  doPlink =0;
  
  if(arguments->argc==2){
    if(!strcmp(arguments->argv[1],"-doPlink")){
      printArg(stdout);
      exit(0);
    }else
      return;
